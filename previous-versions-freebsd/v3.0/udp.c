					...
					uint32_t ttl, uint8_t lcount, uint32_t version, uint8_t A, uint8_t act)
{
	...
	int 
udp_register_add_locator(void *data, struct map_entry * e)
{
	return udp_reply_add_locator(data, e);
}
...
udp_reply_add_locator(void *data, struct map_entry * e)
{
	...
	if( (_fncs & _FNC_XTR) && lisp_te && e->pe){
		...
		while(ptr != &e->pe->tail){
			...
			lcaf->payload_len = ((char *)rpk->curs - (char *)lcaf) - sizeof(struct lcaf_hdr);
			lc++;
			ptr = ptr->next;
		}	
	}
...
general_register_process(void * data)
{
	...
	for (; ;){
		...
		while(ptr != &etr_db->tail){
			...
			while(_iter != &l->tail){
				e = (struct map_entry*)_iter->data;
				udp_register_add_locator(rpk, e);
				_iter = _iter->next;
			}
			ptr = ptr->next;
		}	
		...
		while(ptr != &xtr_ms->tail){
			...
			memcpy(pkbuf, rpk->buf,rpk->buf_len);
			HMAC_SHA1_Init(&ctx);
			HMAC_SHA1_UpdateKey(&ctx, (unsigned char *)ms->key, strlen((char *)ms->key));
			HMAC_SHA1_EndKey(&ctx);
			HMAC_SHA1_StartMessage(&ctx);
			HMAC_SHA1_UpdateMessage(&ctx, pkbuf,rpk->buf_len);
			HMAC_SHA1_EndMessage(buf, &ctx);
			for (i = 0; i < 20; i++){
...
read_rec(union map_reply_record_generic * rec)
{
	...
	while(lcount--){
		bzero(buf, BSIZE);
		//if( (_fncs & (_FNC_XTR| _FNC_RTR ) ) && lisp_te && loc->rloc->pe){
			// struct pe_entry *pe;
			// struct list_entry_t *ptr, *p;
			// struct 
			// ptr = loc->rloc->pe->head.next;
			// while(ptr != &loc->rloc->pe->tail){
				// pe = (struct pe_entry *)ptr->data;
				// p = pe->hop->head.next;
				// while(p != &pe->hop->tail){
...
opl_add_mapp(void * buf, struct db_node * mapp)
{
	...
	if( (mapp->p.prefixlen > 0) && ( mapp->p.prefixlen  < mm) ){
		if( (l = opl_mask2sockaddr(p->prefixlen, p->family, (union sockunion *)mcm)) <=0){
			fprintf(OUTPUT_ERROR, "subnetmask not correct\n");
			return -1;
		}
		l = SS_SIZE(skp);
		mcm = CO(mcm,l);		
...
opl_get(int s, struct db_node *mapp, int db, struct db_node *rs)
{
	...
	/*print eid to test */
	// char ip[INET6_ADDRSTRLEN];
	// inet_ntop(rs->p.family, (void *)&rs->p.u.prefix,ip,INET6_ADDRSTRLEN);
	// printf("EID\t:\t%s/%d\n",ip,rs->p.prefixlen);
	/*end test */
	/*get Rloc if exist */
	if ( (mhdr->map_addrs & MAPA_RLOC ) > 0) {
		...
		for (i = 0; i < mhdr->map_rloc_count ; i++) {
			...
			/*print to test */
			// if(re->rloc.sa.sa_family == AF_INET6){
				// inet_ntop(re->rloc.sin6.sin6_family, &re->rloc.sin6.sin6_addr,ip,INET6_ADDRSTRLEN);
			// }
			// else
				// inet_ntop(re->rloc.sin.sin_family, &re->rloc.sin.sin_addr,ip,INET6_ADDRSTRLEN);
				
			// printf("RLOC\t:%s\tP:%d\tW:%d\tFlags:%s %s\n",ip, re->priority, re->weight, 
					// re->r?"RLOCF_UP":"", re->L?"RLOCF_LIF":"");
			/* end test */
