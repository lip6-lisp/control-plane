...
{
	int rt = -1;
	if(_pk_req_pool[pq_tail] != NULL){
		printf("Wait to input queue has free entry.....\n");
		pthread_mutex_lock(&pq_get_lock);
		pthread_cond_wait(&pq_cv, &pq_get_lock);
	}
	else{
		rt = pq_tail;
		pq_tail = (pq_tail + 1)% PK_POOL_MAX;
	}
	printf("Place:%d\n",rt);
	return rt;
}
...
{
	int rt = -1;
	if(_pk_rpl_pool[pq_tail] != NULL){
		printf("Wait to output queue has free entry.....\n");
		pthread_mutex_lock(&pr_get_lock);
		pthread_cond_wait(&pr_cv, &pr_get_lock);
	}
	else{
		pthread_mutex_lock(&pr_lock);
		pr_no++;
		rt = pr_tail;
		pr_tail = (pr_tail + 1)% PK_POOL_MAX;
...
_rm_rpl(void * entry)
{
	...
	uint32_t
_free_req_pool_place(uint32_t id, void (* fnc)(void *))
{
	if(_pk_req_pool[id]){
		fnc((void *)_pk_req_pool[id]);	
		free(_pk_req_pool[id]);
	}
	_pk_req_pool[id] = NULL;
	_pk_work_pool[id] = 0;
	...
	uint32_t
_free_rpl_pool_place(uint32_t id, void (* fnc)(void *))
{
	pthread_mutex_lock(&pr_lock);
	pr_no--;
	fnc((void *)_pk_rpl_pool[id]);	
	free(_pk_rpl_pool[id]);
	_pk_rpl_pool[id] = NULL;
	pthread_mutex_unlock(&pr_lock);
	pthread_cond_signal(&pr_cv);
	return 0;
}
/*
...
udp_reply_add(uint32_t request_id)
{
	...
	if( (reply_id = udp_new_reply_entry(request_id)) < 0)
	{
		_free_req_pool_place(request_id, _rm_req);
		return -1;
	}
...
udp_reply_terminate(uint32_t id)
{
	...
	if(socket){
		if(sendto(socket, (char *)_pk_rpl_pool[id]->buf, _pk_rpl_pool[id]->buf_len, 0, (struct sockaddr *)&(local.sa), slen) == -1){
			fprintf(OUTPUT_STREAM, "failed\n");
			perror("sendto( )");
			_free_rpl_pool_place(id, _rm_rpl);
			return (FALSE);
		}
	}	
	else{
...
udp_referral_terminate(uint32_t id)
{
	...
	if(socket){
		//hexout(_pk_rpl_pool[id]->buf,_pk_rpl_pool[id]->buf_len);
...
{
	_free_req_pool_place(id, _rm_req);
	return (TRUE);
}
		...
		const struct prefix * eid)
{
	...
	switch(eid->family){
		case AF_INET:
			...
			//rec->record.afi = htons(16387);
			//rec->record.type = 2;
			//rec->record.length = htons(6 + sizeof(struct in_addr));			
			rec->record.eid_prefix_afi = htons(LISP_AFI_IP);
			memcpy(&rec->record.eid_prefix, &eid->u.prefix4, sizeof(struct in_addr));

			/* EID prefix is an IPv4 so 32 bits (4 bytes) */
			//_pk_rpl_pool[reply_id]->curs = (void *)CO(rec, 2 + 12 +2 + 4);
			_pk_rpl_pool[reply_id]->curs = (void *)CO(rec, sizeof(struct map_request_record));
			break;
		case AF_INET6:
			rec->record6.eid_mask_len = eid->prefixlen;
			//rec->record.afi = htons(16387);
			//rec->record.type = 2;
			//rec->record.length = htons(6 + sizeof(struct in6_addr));			
			...
			//_pk_rpl_pool[reply_id]->curs = (void *)CO(rec, 2 + 12 + 2 + 16);
			_pk_rpl_pool[reply_id]->curs = (void *)CO(rec, sizeof(struct map_request_record6));
			break;
		default:
...
udp_request_ddt_terminate(uint32_t id, const union sockunion * server, char terminal)
{
	...
	*/	
	...
	//close(skt);
	if(terminal){
		_free_req_pool_place(_pk_rpl_pool[id]->request_id, _rm_req);
	}
	_free_rpl_pool_place(id, _rm_rpl);
...
udp_get_pk(int sockfd, socklen_t slen)
{
	...
	p  = calloc(1,sizeof(struct pk_req_entry));
	//printf("Orginal package:\n");
	//hexout(buf,pk_len);
	p->ttl = 0;
	p->hop = 0;
	p->itr = p->eid = NULL;	
...
_lisp_process(void *data)
{
	...
	switch(lh->type){
		...
		case LISP_TYPE_MAP_NOTIFY:
			...
			_free_req_pool_place(pk_id, _rm_req);
			break;
		case LISP_TYPE_MAP_REQUEST:
			/* Parse request message */
			rt = _request(pk_id);
			if(rt <= 0){
				fprintf(OUTPUT_STREAM, "Not a map-request.....Drop!\n");
				_free_req_pool_place( pk_id, _rm_req);
				break;
			}
			if(_fncs & _FNC_XTR){
	...
	void *
_lisp_worker(void *data)
{
	thr_pool_t *cpp;
	cpp = thr_pool_create(min_thread,max_thread,linger_thread, NULL);
	int ptr[2*PK_POOL_MAX];
	int i = 0;
	while(1){		
		if(_pk_req_pool[pq_cur] == NULL || _pk_work_pool[pq_cur]){
			printf("Wait input....\n");
		...
		ptr[i] = pq_cur;
		_pk_work_pool[pq_cur] = 1;
		thr_pool_queue(cpp, _lisp_process, (void *)&ptr[i]);				
		i = (i+1)%(2*PK_POOL_MAX);
		pq_cur = (pq_cur+1) % PK_POOL_MAX;		
	}
	pthread_exit(NULL);
}	
...
udp_start_communication(void * context)
{
	...
	socklen_t slen = 0;
	pq_cur = pq_tail = pq_no = pr_cur = pr_tail = pr_no = 0;
	pthread_mutex_init(&pq_lock, NULL);
	pthread_mutex_init(&pq_get_lock, NULL);
	pthread_mutex_init(&pr_lock, NULL);
	pthread_mutex_init(&pr_get_lock, NULL);
	pthread_mutex_init(&work_lock, NULL);
	pthread_cond_init(&pq_cv,NULL);
	pthread_cond_init(&pq_cv,NULL);
	...
	pthread_create(&_thr_lisp_process, NULL, _lisp_worker, NULL);	
...
/* Map-notify */
	int
_register_notify(uint32_t request_id, struct site_info *site )
{
	...
	for (i = 0; i < auth_len; i++)
		lcm->auth_data[i]=0;
	pkbuf = calloc(p->buf_len, sizeof(char));	
	memcpy(pkbuf,buf,p->buf_len);
	HMAC_SHA1_Init(&ctx);
	HMAC_SHA1_UpdateKey(&ctx, site->key, strlen((char *)site->key));
	HMAC_SHA1_EndKey(&ctx);
	HMAC_SHA1_StartMessage(&ctx);
	HMAC_SHA1_UpdateMessage(&ctx, pkbuf,p->buf_len);
...
_register(uint32_t request_id)
{
	...
	void * packet = _pk_req_pool[request_id]->buf;	
	int rt;
	int pkg_len = _pk_req_pool[request_id]->buf_len;
	...
	if ( (rt = _ms_validate_register(ms_db, packet, pkg_len, (void *)&site)) >=0 )
	{
		...
		if(lcm->want_map_notify && site->data){
			_register_notify(request_id, site->data);
		}
		return 1;
	}
	return 0;
...
_ms_recal_hashing(const void *packet, int pk_len, void * key, void *rt, int no_nonce)
{
	...
	/* if(!no_nonce){
		printf("Auth data::\n");
		hexout(map_register->auth_data, auth_len);
	} */	
...
_ms_validate_register(struct lisp_db * db, const void * packet, int pkg_len, void **site_ptr)
{
	...
	*site_ptr = site;
	...
	//hexout((char *)info_hmac, auth_len);
	//hexout((char *)s_hmac, auth_len);
...
general_register_process(void * data)
{
	...
	for (; ;){
		...
		_free_rpl_pool_place(register_id, _rm_rpl);
		sleep(60);
	}
	return NULL;
}
...
/*Add new EID to poll*/
	void 
mr_new_lookup(uint32_t request_id,struct communication_fct * fct,struct db_node *rn)
{
    int i,e,r,r6;
	...
	pkg_len = _pk_req_pool[request_id]->buf_len - (_pk_req_pool[request_id]->lh - _pk_req_pool[request_id]->buf);
	mr_lookups[i].orgi_pkg = calloc(pkg_len, sizeof(char));
	memcpy(mr_lookups[i].orgi_pkg, _pk_req_pool[request_id]->lh, pkg_len);
...
int 
pending_request(uint32_t request_id, struct communication_fct * fct, struct db_node *rn)
{
	uint64_t nonce;
	uint32_t *nonce0, *nonce1;
	int i, l;
	...
	for (i = 0; i < MAX_LOOKUPS; i++) {
		...
		//printf("Compare::%x:%x:%x:%x\n",*nonce0,*nonce1,mr_lookups[i].nonce0,mr_lookups[i].nonce1);
	...
	if( l >= 0){
		...
		//fprintf(OUTPUT_STREAM, "Add request to queue\n");
		mr_new_lookup(request_id,fct,rn);
	}
	//_free_req_pool_place(request_id, _rm_req);	
	return 0;
}
...
free_lookups(int idx)
{
	...
	list_destroy(mr_lookups[idx].rlocs,NULL);	
	...
	while(rcount--){
		...
		switch (rec->record.act) {
			...
			case LISP_REFERRAL_DELEGATION_HOLE:
				...
				return 0;
				break;
			case LISP_REFERRAL_NOTE_AUTHORITATIVE:
				//clear cache
				free_lookups(idx);
				return 0;
				break;			
