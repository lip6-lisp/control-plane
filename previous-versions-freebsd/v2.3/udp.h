...
int udp_referral_add_record(uint32_t id, uint32_t iid, struct prefix * p, 
...
int udp_reply_add_record(uint32_t id, struct prefix * p, 
...
int udp_request_get_itr(uint32_t id, union sockunion * itr, int afi);
...
int udp_request_ddt_terminate(uint32_t id, const union sockunion * server, 
							char terminal);
