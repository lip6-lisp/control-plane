...
generic_mapping_add_rloc(void *mapping, struct map_entry *entry)
{
	...
	list_insert(locs, entry, _map_entry_cmp);
...
/* make a negative referral reply */
	int
_send_negative_map_referral(void *data, struct communication_fct *fct, \
...
/* make a map-referral reply */
	int
_send_map_referral(void *data, struct communication_fct *fct, \
		struct db_node *rn)
{
...
/* make a negative map-reply */
	int
_send_negative_map_reply(void *data, struct communication_fct *fct, \
...
/* make a map-reply */
	int
_send_map_reply(void *data, struct communication_fct *fct, \
		struct db_node *rn)
{
	struct map_entry *e = NULL;
	struct list_entry_t *_iter;
...
/* when request EID (REID) match an negative node (NEG_NODE)
 * reply contain an EID which
 * less-specific than REID
 * more-specific than NEG_NODE
 * not overlap any child of NEG_NODE
 */
#ifndef PNBBY
#define PNBBY 8
#endif /* PNBBY */
		...
		if (ms_node_is_type(node, _MAPP)) {
			...
			if (!ms_node_is_referral(node)) {
				...
				if (_fncs & _FNC_MS) {
					...
					_send_map_referral(pke, fct, node);

					_forward_to_etr(pke,node);
			...
			if (_fncs & _FNC_NODE) {
				...
				if (((struct mapping_flags *)node->flags)->A ) {
					 ...
					 */
					get_hole_eid(&p, rn, &p);
					_send_negative_map_referral(pke, fct, rn, &p, 0, 0,
										LISP_REFERRAL_NOT_AUTHORITATIVE, 0, 1);
					return (TRUE);
				}
			}
		}
			...
			if (_fncs & _FNC_MS || _fncs & _FNC_NODE) {
				...
				if (node == table->top) {
					...
					if (node->flags && ms_node_is_type(node, _GEID | _GREID))
						_send_negative_map_referral(pke, fct, rn, &p, 15, 0,
											LISP_REFERRAL_DELEGATION_HOLE,0, 1 );
					else
						_send_negative_map_referral(pke, fct, rn, &p, 0, 0,
											LISP_REFERRAL_NOT_AUTHORITATIVE, 0, 1);
					return TRUE;
				}else{
						int nflg = ((struct mapping_flags *)node->flags)->range;
						if ((nflg & _EID) && (_fncs & _FNC_MS) ) {/* EID not is registered */
							get_hole_eid(&p, rn, &p);
							_send_negative_map_referral(pke, fct, rn, &p, 1, 1,
											LISP_REFERRAL_MS_NOT_REGISTERED, 0, 1);
							return TRUE;
						} else if ((nflg & _GEID) && (_fncs & _FNC_MS)) { /* EID not assign for any ETR */
							/* in ieft, this case is same as _EID, but I think TTL must longer*/
							get_hole_eid(&p, rn, &p);
							_send_negative_map_referral(pke, fct, rn, &p, 15, 1,
											LISP_REFERRAL_DELEGATION_HOLE, 0, 1);
							return TRUE;
						}else if ((nflg & _GREID) && (_fncs & _FNC_NODE)) {/* EID not delegated */
							get_hole_eid(&p, rn, &p);
							_send_negative_map_referral(pke, fct, rn, &p, 15, 0,
											LISP_REFERRAL_DELEGATION_HOLE,0, 1 );
							return TRUE;
	...
	/* Received non DDT request */
		...
		if (ms_node_is_type(node, _MAPP)) {
			...
			if (!ms_node_is_referral(node)) {
				if (_fncs & _FNC_MS) {
					if (ms_node_is_proxy_reply(node)) {
						return _send_map_reply(pke, fct, node);
					}
					else{
						cp_log(LDEBUG, "Forwarding to ETR\n");

						return _forward_to_etr(pke,node);
					}
				}
				return FALSE;
			}
			...
			if (_fncs & _FNC_MS) {
				...
				if (ms_node_is_type(node,_EID))
					_send_negative_map_reply(pke, fct, node, &p, 60, 1, 1, 0);
				else
					_send_negative_map_reply(pke, fct, node, &p, 900, 1, 1, 0);
				return TRUE;
			}
			if (_fncs & _FNC_MR) {
				pke->ref_cnt++;
